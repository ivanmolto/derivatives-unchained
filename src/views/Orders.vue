              ...
              <div class="shadow-lg border border-gray-300">
                <div class="relative pb-5/6">
                  <span
                    ...
                    :class="
                      order.side != 'Buy' ? 'bg-red-500' : 'bg-green-500'
                    "
                  >
                    {{ order.side }}
                  </span>
...
<script>
import orderSubscription from "../graphql/orders.subscription.gql";
import { useSubscription, useResult } from "@vue/apollo-composable";

export default {
  name: "Orders",
  setup() {
    const { result, loading, error } = useSubscription(orderSubscription);

    const orders = useResult(result, null, (data) => data.orders);
